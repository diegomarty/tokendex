...
// Cases where V8's `new Date()` disagrees with the correct reading, measured on Node 20.
// These are the reason this parser exists at all — if it ever gets "simplified" to
// `new Date(value)`, these fail.
describe('parseISO8601 — V8 divergences', () => {
  it('truncates over-long fractions instead of misreading them', () => {
    // V8's `new Date` yields .344 here; truncating to 3 digits gives the correct .034.
    expect(parseISO8601('2026-06-10T11:10:00.0344645678Z')).toBe(parseISO8601('2026-06-10T11:10:00.034Z'))
  })

  it('right-pads short fractions (".3" is 300ms, not 3ms)', () => {
    expect(parseISO8601('2026-06-10T11:10:00.3Z')).toBe(parseISO8601('2026-06-10T11:10:00.300Z'))
  })
  ...
  it('honours non-UTC offsets', () => {
    expect(parseISO8601('2026-06-10T11:10:00+09:00')).toBe(parseISO8601('2026-06-10T02:10:00Z'))
  })
})
