...
import { daily, enrichmentScanStart, entryTotal, monthKey, period, todayKey } from '../src/core/usage/entry.js'
...
async function pass(label: string) {
  ...
  console.log(`\n  ${label}`)
  console.log(`    claude   ${String(Math.round(tClaude - t0)).padStart(6)} ms   ${grouped(claude.length, 'es-ES').padStart(7)} entradas`)
  console.log(`    codex    ${String(Math.round(tCodex - tClaude)).padStart(6)} ms   ${grouped(codex.length, 'es-ES').padStart(7)} entradas`)
  console.log(`    TOTAL    ${String(Math.round(tCodex - t0)).padStart(6)} ms`)
  return { claude, codex }
}

const first = await pass('1ª pasada (caché en frío)')
const second = await pass('2ª pasada (instancia nueva: carga snapshot + escaneo)')
...
// This is the number that actually repeats every 120 s.
{
  ...
  console.log('\n  Régimen (misma instancia, como en la extensión)')
  for (let i = 1; i <= 3; i++) {
    const t0 = performance.now()
    await cache.claudeEntries(since)
    await cache.codexEntries(since)
    console.log(`    refresco ${i}   ${String(Math.round(performance.now() - t0)).padStart(5)} ms`)
  }
}
...
console.log(`\n  Uso combinado`)
console.log(`    hoy        ${d ? compact(d.totalTokens) : '—'} tokens   ${d ? cost(d.totalCost) : ''}`)
console.log(`    este mes   ${compact(m.totalTokens)} tokens   ${cost(m.totalCost)}`)
console.log(`\n  paridad entre pasadas: ${first.claude.length === second.claude.length && first.codex.length === second.codex.length ? 'OK (mismos conteos)' : '¡DIFIEREN!'}\n`)
